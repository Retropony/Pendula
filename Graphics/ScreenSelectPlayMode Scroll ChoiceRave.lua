local t = Def.ActorFrame{};
	...
	t[#t+1] = LoadActor("_clock-selection")..{
		...
		GainFocusCommand=cmd(linear,0.1;diffuse,color("#f700e8"));
	...
	t[#t+1] = LoadActor(THEME:GetPathG("","_pointer/om-battle") )..{
		InitCommand=cmd(zoom,0.75;diffusealpha,1;rotationz,30);
		GainFocusCommand=cmd(linear,0.1;diffusealpha,0);
		LoseFocusCommand=cmd(linear,0.1;diffusealpha,1);
	};
	t[#t+1] = LoadActor(THEME:GetPathG("","_pointer/i-common") )..{
		InitCommand=cmd(zoom,0.75;diffusealpha,0;diffuse,color("#f700e8"));
	...
	t[#t+1] = LoadActor(THEME:GetPathG("","_pointer/im-battle") )..{
		InitCommand=cmd(zoom,0.75;diffusealpha,0;diffuse,color("#f700e8");rotationz,30);
		GainFocusCommand=cmd(linear,0.1;diffusealpha,1);
		LoseFocusCommand=cmd(linear,0.1;diffusealpha,0);
	};
	t[#t+1] = LoadActor(THEME:GetPathG("","_mode/o-battle") )..{
		InitCommand=cmd(zoom,0.75);
	};
	t[#t+1] = LoadActor(THEME:GetPathG("","_mode/i-battle") )..{
