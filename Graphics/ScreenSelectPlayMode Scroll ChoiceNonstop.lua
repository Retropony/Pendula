local t = Def.ActorFrame{};
	...
	t[#t+1] = LoadActor("_clock-selection")..{
		...
		GainFocusCommand=cmd(linear,0.1;diffuse,color("#f4f700"));
		LoseFocusCommand=cmd(linear,0.1;diffuse,color("#ffffff"));
	};
	t[#t+1] = LoadActor(THEME:GetPathG("","_pointer/ob-battle") )..{
	...
	t[#t+1] = LoadActor(THEME:GetPathG("","_pointer/om-battle") )..{
		InitCommand=cmd(zoom,0.75;diffusealpha,1;rotationz,180);
		GainFocusCommand=cmd(linear,0.1;diffusealpha,0);
		LoseFocusCommand=cmd(linear,0.1;diffusealpha,1);
	};
	t[#t+1] = LoadActor(THEME:GetPathG("","_pointer/i-common") )..{
		InitCommand=cmd(zoom,0.75;diffusealpha,0;diffuse,color("#f4f700"));
	...
	t[#t+1] = LoadActor(THEME:GetPathG("","_pointer/ib-battle") )..{
		InitCommand=cmd(zoom,0.75;diffusealpha,0;diffuse,color("#f4f700"));
		GainFocusCommand=cmd(spin;effectmagnitude,0,0,12;linear,0.1;diffusealpha,1);
		LoseFocusCommand=cmd(stopeffect;linear,0.1;diffusealpha,0;sleep,1/60;rotationz,0);
	};
	t[#t+1] = LoadActor(THEME:GetPathG("","_pointer/im-battle") )..{
		InitCommand=cmd(zoom,0.75;diffusealpha,0;diffuse,color("#f4f700");rotationz,180);
		GainFocusCommand=cmd(linear,0.1;diffusealpha,1);
		LoseFocusCommand=cmd(linear,0.1;diffusealpha,0);
	};
	t[#t+1] = LoadActor(THEME:GetPathG("","_mode/o-class") )..{
		InitCommand=cmd(zoom,0.75);
	};
	t[#t+1] = LoadActor(THEME:GetPathG("","_mode/i-class") )..{
